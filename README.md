...
一个面向神绮爱与观众的非商业粉丝网站。推荐单、美食家、许愿箱与棉花糖共同组成公开互动空间；神绮爱既能处理观众投稿，也能直接撰写自己的内容。
...
- 顶部栏目为推荐单、许愿箱、美食家、棉花糖；推荐单下拉保留书籍、漫画、电影、动漫、游戏等分类
- 美食家支持观众和神绮爱投稿店铺、菜品与推荐理由，沿用推荐/不推荐、评论和排序能力
- 许愿箱用于征集直播愿望，未完成优先展示，神绮爱可审核、回应并标记完成
- 10 分制神绮爱评分、按时间或评分排序；任何排序都始终优先显示人工置顶
- 每个公开作品支持一人一票的“推荐 / 不推荐”，按净推荐数排序，并在首页卡片显示评价数
- 独立作品详情页与用户评论；评论可选中文字标记局部剧透，悬停或键盘聚焦后显示
- 投稿与神绮爱回复中的 BV 号自动变成 bilibili.com 视频链接
- 搜索、分页、软删除与恢复
- `/marshmallow` 棉花糖投稿与公开墙；个人棉花糖统一在“我的投稿”管理，未读前可修改或删除
- 棉花糖直播展示台：最早优先、上下切换、跳过、展示、已读和防误触移除
- 棉花糖已读、上墙和下架独立操作；已读或主播回信不会自动公开，私密棉花糖不能上墙或进入直播展示区
- 主播可回复和修改棉花糖回信，投稿人在“我的投稿”查看并收到站内提醒；公开回信随棉花糖上墙，下架后一起隐藏，内容与点赞保留
- 小羊老师电台：左上角小羊图标进入，按每期主题 → 本期目录 → 详细正文逐层阅读；主播在后台整理故事和收录的观众投稿
- 神绮爱感想、回推首页和站内通知；普通编辑不会重复通知
- CSS Variables 主题系统、实时预览、可选 Vercel Blob 背景上传
- 站名会同步浏览器标签标题；首页和四个栏目主要文案可由神绮爱在后台修改并保存在数据库中
- 用户管理显示注册、启用和待审核数量；管理员可生成一次性密码，用户登录后必须立即设置新密码
...
- 访客点击左上角小羊图标进入 `/radio`，这里只显示已发布的每期主题，不在首页铺开故事。
- 点击主题进入 `/radio/[episodeId]` 查看本期目录，再点击条目进入 `/radio/[episodeId]/[entryId]` 阅读完整正文。
- 主播登录后在 `/host/radio` 创建期数、设置主题，录入故事或从其他渠道收录的观众投稿，并按目录顺序整理内容。观众不在本站提交电台投稿。
- 每期先保存为草稿，发布后访客才能访问；取消发布后该期及正文一起隐藏。隐藏入口不等于密码保护，已发布页面可通过链接访问。
- 页面复用现有配色、背景、卡片和表单样式。本版入口使用导航栏小羊图标，背景图中的羊暂未添加点击区域。

启用前先检查并运行新增的数据库迁移（`npm run db:migrate`），以创建电台期数和内容表。已有站点应先备份，再迁移及部署新版本；此功能不会自动连接数据库执行迁移。
...
本项目的 Neon 位于 AWS 新加坡区，`vercel.json` 因此把 Vercel Functions 固定到 `sin1`，避免每次数据库查询跨洲往返。如果以后迁移 Neon 区域，请同步修改这个配置。不要在构建命令中自动执行不可逆 migration；先审查 migration，再由维护者执行。

上线后建议在 Vercel Firewall 添加一条针对 `/login`、`/register` 的 POST 限流规则。代码内限流是单实例的第二道防线，无法替代分布式 WAF。
...
- 页面主体只使用自有 CSS、系统字体和开源 `lucide-react` 图标。Lucide 使用 ISC License，源自 Feather 的部分使用 MIT License；两种许可都允许当前非商业站点使用与再分发。
- 默认页面不加载外部字体、图标 CDN、影视封面、动漫素材或游戏截图。
- `public/og-v2.png` 是通过 OpenAI 图像生成制作的原创社交分享图，不是第三方版权素材。它只用于链接预览，不出现在站内作品卡片中；旧版 `public/og.png` 留作回退与替换参考。
- 若要替换分享图，使用一张 `1200 × 630` 的自有或已授权 PNG 覆盖 `public/og-v2.png`，并同步检查 `app/layout.tsx` 中的标题与替代文本。
...
- XLSX 的「电台期数」「电台内容」工作表与完整 ZIP 均包含电台草稿和已发布内容；长篇正文在 XLSX 中分为「正文」「正文（续）」两列，避免单元格长度限制。ZIP schema 15 必须包含对应数据文件及校验值；schema 13、14 的旧备份仍可导入，缺少的电台数据视为空。
