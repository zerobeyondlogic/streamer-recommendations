...
const statuses: { value: MarshmallowHostStatus; label: string }[] = [
  { value: "pending", label: "待处理" }, { value: "published", label: "已上墙" },
  { value: "private", label: "已收下（私密）" }, { value: "deleted", label: "已移除" }, { value: "all", label: "全部" },
];
...
export default async function MarshmallowManagerPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <>
    <header className="host-heading"><div><span className="eyebrow">棉花糖</span><h1>收件箱</h1><p>从待处理开始，公开或私密收下。</p></div><Link className="button ghost" href="/host/marshmallows/stage"><Eye aria-hidden="true"/> 进入展示台</Link></header>
    ...
    <div className="marshmallow-manager-list">{items.map((item) => <article className={`panel marshmallow-manager-card ${item.deletedAt ? "is-deleted" : ""}`} key={item.id}>
      ...
      <div className="marshmallow-state-line"><span>{item.deletedAt ? `已移除 · ${formatDate(item.deletedAt)}` : item.readAt ? item.publishedAt ? `已上墙 · ${formatDate(item.readAt)}` : `已收下 · ${formatDate(item.readAt)}` : "待处理"}</span>{item.publishedAt ? <span>公开于 {formatDate(item.publishedAt)}</span> : null}</div>
