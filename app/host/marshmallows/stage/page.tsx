...
export default async function MarshmallowStagePage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <>
    <header className="host-heading compact"><div><span className="eyebrow">Live display</span><h1>棉花糖展示台</h1><p>展示后可标记已读，或另行公开上墙。</p></div><Link className="button ghost" href="/host/marshmallows">返回管理</Link></header>
    <Notice>{params.error}</Notice><Notice type="success">{params.success}</Notice>
    {stage ? <MarshmallowStage
      key={stage.current.id}
