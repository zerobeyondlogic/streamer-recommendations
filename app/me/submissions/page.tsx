...
export default async function MySubmissionsPage({ searchParams }: { searchParams: Promise<{ minePage?: string; marshmallow?: string; error?: string; success?: string }> }) {
  ...
  return <div className="form-page wide personal-records-page">
    ...
    <section id="work-submissions" className="personal-record-section" aria-labelledby="work-submissions-title">
      ...
      <div className="record-list">{items.map((item) => { const kind = submissionKind(item.category); return <article className="panel record-card" id={`submission-${item.id}`} key={item.id}>
        <div className="record-main"><div className="card-top"><span className={`collection-pill collection-pill-${kind}`}>{submissionKindLabels[kind]}</span><span className={`category category-${item.category}`}>{categoryLabels[item.category]}</span>{item.anonymousPublic ? <span className="pin">匿名展示</span> : null}{item.score ? <span className="score compact-score"><b>{item.score}</b><small>/10</small></span> : null}{item.unread ? <span className="unread-pill">有新回复</span> : null}</div><h2>{item.title}</h2><div className="record-meta"><span>{formatDate(item.createdAt)}</span><span>{item.hostReadAt ? "已查看" : "未查看"}</span><span>{item.publishedAt ? "已公开" : "未公开"}</span><span>{contentStatusLabel(item.category, item.contentStatus)}</span></div>{item.reply ? <div className="host-reply"><strong>{kind === "wish" ? "神绮爱回应" : kind === "food" ? "神绮爱试吃感想" : "神绮爱感想"}</strong><p><BvText>{item.reply}</BvText></p></div> : null}</div>
        <div className="record-actions"><SubmissionContentEditor item={item} returnTo={`/me/submissions#submission-${item.id}`}/>{!item.hostReadAt ? <form action={deleteOwnSubmissionAction}><input type="hidden" name="submissionId" value={item.id}/><ConfirmSubmit label="撤回" title="撤回这条投稿？" description="撤回后会永久删除，无法恢复。" confirmLabel="确认撤回"/></form> : null}{item.publishedAt ? <Link className="button small ghost" href={`/submission/${item.id}`}>查看详情</Link> : null}</div>
      </article>; })}</div>
      ...
      <div className="my-marshmallow-list">{marshmallows.items.map((item) => {
        ...
        return <article className={`panel my-marshmallow-card ${editable ? "is-editable" : "is-locked"}`} id={`my-marshmallow-${item.id}`} key={item.id}>
          ...
          {editable ? <div className="my-marshmallow-actions">
            <details className="my-marshmallow-editor"><summary><Pencil aria-hidden="true"/> 修改</summary><form className="stack" action={updateOwnMarshmallowAction}><input name="marshmallowId" type="hidden" value={item.id}/><label>内容<textarea name="content" maxLength={10000} required defaultValue={item.content}/></label><label className="checkbox"><input name="allowPublic" type="checkbox" defaultChecked={item.allowPublic}/><span>已读后允许上墙</span></label><button className="button small primary" type="submit">保存并重新投递</button><span className="helper">首次投递时间不变。</span></form></details>
