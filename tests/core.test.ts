...
describe("账号与输入安全",()=>{
  ...
  it("棉花糖默认私密、拒绝空内容并限制为 10000 字",()=>{expect(marshmallowSchema.parse({content:"  想说的话  "})).toEqual({content:"想说的话",allowPublic:false});expect(marshmallowSchema.safeParse({content:"   "}).success).toBe(false);expect(marshmallowSchema.safeParse({content:"糖".repeat(10000)}).success).toBe(true);expect(marshmallowSchema.safeParse({content:"糖".repeat(10001)}).success).toBe(false)});
