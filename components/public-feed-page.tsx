      ...
      <div className="feed-grid">
        {feed.map((item) => {
          ...
          return <article className={`feed-card feed-card-${itemKind} ${item.pinnedAt ? "is-pinned" : ""} ${item.source === "host" ? "is-host-authored" : ""}`} key={item.id} id={item.id}>
            ...
            {item.reply ? <div className="host-reply"><div className="reply-title"><span className="avatar">爱</span><strong>{copy[itemKind].reply}</strong></div><p><BvText>{item.reply}</BvText></p><time>{formatDate(item.replyPublishedAt)}</time></div> : <div className="waiting-reply">{itemKind === "wish" ? "等待神绮爱回应" : "暂无感想"}</div>}
