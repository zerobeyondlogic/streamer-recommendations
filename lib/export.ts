...
export async function createXlsxExport() {
  ...
  const db = getDb();
  const [submissionRows, userRows, notificationRows, settingsRows, marshmallowRows] = await Promise.all([
    db.select().from(submissions), db.select().from(users), db.select().from(notifications), db.select().from(siteSettings), db.select().from(marshmallows),
  ]);
...
export async function validateBackup(bytes: Buffer) {
  const JSZip = (await import("jszip")).default; const zip=await JSZip.loadAsync(bytes); const required=["manifest.json","schema-version.txt","users.json","submissions.json","marshmallows.json","host-replies.json","notifications.json","site-settings.json","activity-logs.json","checksums.sha256"];
  ...
  return { manifest:await read<Record<string,unknown>>("manifest.json"), users:await read<Record<string,unknown>[]>("users.json"), submissions:await read<Record<string,unknown>[]>("submissions.json"), marshmallows:await read<Record<string,unknown>[]>("marshmallows.json"), replies:await read<Record<string,unknown>[]>("host-replies.json"), notifications:await read<Record<string,unknown>[]>("notifications.json"), settings:await read<Record<string,unknown>[]>("site-settings.json"), logs:await read<Record<string,unknown>[]>("activity-logs.json") };
}
