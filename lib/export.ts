...
export async function createXlsxExport() {
  ...
  const db = getDb();
  const [submissionRows, userRows, notificationRows, settingsRows] = await Promise.all([
    db.select().from(submissions), db.select().from(users), db.select().from(notifications), db.select().from(siteSettings),
  ]);
...
export async function createFullBackup() {
  ...
  const db = getDb();
  const [userRows, submissionRows, replyRows, notificationRows, settingsRows, logRows] = await Promise.all([
    db.select().from(users), db.select().from(submissions), db.select().from(hostReplies), db.select().from(notifications), db.select().from(siteSettings), db.select().from(activityLogs),
  ]);
  const files = new Map<string, Buffer>();
  files.set("manifest.json", json({ product: "streamer-recommendations", schemaVersion: SCHEMA_VERSION, createdAt: new Date().toISOString(), sensitive: true, counts: { users:userRows.length, submissions:submissionRows.length, replies:replyRows.length, notifications:notificationRows.length, settings:settingsRows.length, activityLogs:logRows.length } }));
  files.set("schema-version.txt", Buffer.from(`${SCHEMA_VERSION}\n`));
  files.set("users.json", json(userRows)); files.set("submissions.json", json(submissionRows)); files.set("host-replies.json", json(replyRows)); files.set("notifications.json", json(notificationRows)); files.set("site-settings.json", json(settingsRows)); files.set("activity-logs.json", json(logRows));
...
export async function validateBackup(bytes: Buffer) {
  const JSZip = (await import("jszip")).default; const zip=await JSZip.loadAsync(bytes); const required=["manifest.json","schema-version.txt","users.json","submissions.json","host-replies.json","notifications.json","site-settings.json","activity-logs.json","checksums.sha256"];
