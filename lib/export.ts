...
export async function createXlsxExport() {
  ...
  add("主题设置", ["网站名称","网站副标题","网页图标地址","自定义字体地址","推荐单首页插画地址","背景类型","电脑背景地址","手机背景地址","主色","辅助色","强调色","页面背景色","菜单透明度","菜单毛玻璃","大卡片透明度","大卡片毛玻璃","搜索栏透明度","搜索栏毛玻璃","普通卡片透明度","普通卡片毛玻璃","背景悬浮文字雾气浓度","遮罩强度","更新时间"], settingsRows.map((s) => [s.siteName,s.siteTagline,s.siteIconUrl,s.customFontUrl,s.recommendationHeroImageUrl,s.backgroundType,s.backgroundImageUrl,s.backgroundImageMobileUrl,s.primaryColor,s.secondaryColor,s.accentColor,s.backgroundColor,s.navOpacity,s.navBlur,s.heroOpacity,s.heroBlur,s.filterOpacity,s.filterBlur,s.cardOpacity,s.cardBlur,s.ambientTextMist,s.backgroundOverlay,s.updatedAt]));
