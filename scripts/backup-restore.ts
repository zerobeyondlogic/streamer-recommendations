...
const data=await validateBackup(await readFile(resolve(path)));const counts={users:data.users.length,submissions:data.submissions.length,replies:data.replies.length,notifications:data.notifications.length,settings:data.settings.length,logs:data.logs.length};console.log("即将导入：",counts);
