...
const HOST_BILIBILI_UID="1428849513";
...
try{
  ...
  if(existing){await getDb().update(users).set({role:"host",status:"active",passwordHash,bilibiliUid:HOST_BILIBILI_UID,bilibiliVerifiedAt:new Date(),updatedAt:new Date()}).where(eq(users.id,existing.id));console.log(`已将 ${existing.username} 更新为主播账号。`);}
  else{await getDb().insert(users).values({username:parsed.username,usernameNormalized:normalized,passwordHash,role:"host",status:"active",bilibiliUid:HOST_BILIBILI_UID,bilibiliVerifiedAt:new Date()});console.log(`已创建主播账号 ${parsed.username}。`);}
